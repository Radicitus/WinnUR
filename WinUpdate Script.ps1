...
$msg.IsBodyHTML = $true
$msg.Subject = "Daily powershell update report from $Env:ComputerName"
#END EMAIL SETUP
#-----------------------------
...
If ($Result.Updates.Count -EQ 0) {
    ...
    $ReportFileLocation = (Get-ChildItem "$PSScriptRoot" *_Report.txt -Recurse).fullname
    Add-Content $ReportFile "Report located at: $ReportFileLocation"
    Add-Content $ReportFile "Report Created On: $Today`r"
    Add-Content $ReportFile "==============================================================================`r`n"
    Add-Content $ReportFile "No Updates Required!"
    $attachment = new-object System.Net.Mail.Attachment $ReportFileLocation
    $msg.Attachments.Add($attachment)
    #$msg.Body = Get-Content $Env:ComputerName + "_Report.txt"|Out-String
